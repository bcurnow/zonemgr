...
func teardown(_ *testing.T) {
	...
	validations = plugins.Validations()
}
...
func testValidateZone(t *testing.T, p plugins.ZoneMgrPlugin) {
	if err := p.ValidateZone("noop", &schema.Zone{}); err != nil {
		t.Errorf("unexpected error: %s", err)
	}
}
...
func testNormalizeInvalidName(t *testing.T, tn *testNormalize) {
	...
	rr := &schema.ResourceRecord{Type: tn.rrType, Name: "invalidname" + strings.Repeat("e", 63)}
...
func testNormalizeValueIsIP(t *testing.T, tn *testNormalize) {
	identifier := "name"
	rr := &schema.ResourceRecord{Type: tn.rrType, Value: "not an IP", Name: identifier}
...
// NOTE: setup needs to be called before calling this method!
func testNormalizeValueNotIP(t *testing.T, plugin plugins.ZoneMgrPlugin, pluginType plugins.PluginType, rrType schema.ResourceRecordType) {
	identifier := "name"
	rr := &schema.ResourceRecord{Type: rrType, Value: "not an IP", Name: identifier}

	mockValidations.EXPECT().StandardValidations(identifier, rr, pluginType)
	mockValidations.EXPECT().IsValidNameOrWildcard(identifier, identifier, rrType)
	mockValidations.EXPECT().EnsureNotIP(identifier, rr.Value, rrType).Return(fmt.Errorf("is an IP"))
...
func testNormalizeValueNotFullyQualified(t *testing.T, tn *testNormalize) {
	identifier := "name"
	rr := &schema.ResourceRecord{Type: tn.rrType, Name: "name", Value: "value.example.com."}
	tn.expects(identifier, rr)
	err := tn.plugin.Normalize(identifier, rr)
	if err == nil {
		t.Errorf("expected error")
