...
func (p *SOAPlugin) Normalize(identifier string, rr *schema.ResourceRecord) error {
	...
	if p.config.GenerateSerial {
		...
		nextSerial, err := serial.GetNext(rr.Name)
		if err != nil {
			return err
		}
...
func normalizeValues(identifier string, rr *schema.ResourceRecord, generateSerial bool, serial string) error {
	...
	if numValues == 7 && generateSerial {
		// Replace element 2
		rr.Values[2] = &schema.ResourceRecordValue{Value: serial, Comment: generatedSerialNumberComment}
	}
...
func insertSerial(rr *schema.ResourceRecord, serial string) {
	...
	withSerial := make([]*schema.ResourceRecordValue, 7)

	//Copy indicies 0 and 1 into the array
	copy(withSerial[:2], rr.Values[:2])

	// insert the serial number
	withSerial[2] = &schema.ResourceRecordValue{Value: serial, Comment: generatedSerialNumberComment}
...
func init() {
	registerBuiltIn(plugins.SOA, &SOAPlugin{})
}
